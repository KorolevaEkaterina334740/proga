...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    x = pos[0] - pos[0] % 3
    y = pos[1] - pos[1] % 3
    block = [[grid[x + i][y + j] for j in range(3)] for i in range(3)]
...
def find_empty_positions(
    ...
    flattened_empty_positions = [j for i in empty_positions for j in i]
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    if not empty_pos:
        ...
        poss_val = find_possible_values(grid, empty_pos)
        if poss_val:
            for i in poss_val:
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(1, 10):
        for j in range(1, 10):
            ...
            if col.count(str(i)) != 1 or row.count(str(i)) != 1 or block.count(str(i)) != 1:
                return False
    return True
